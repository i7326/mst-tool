...
var Shell = exports.Shell = function (_eventEmitter) {
  ...
  function Shell() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        ...
        executionPolicy = _ref$executionPolicy === undefined ? 'Bypass' : _ref$executionPolicy,
    ...
    _this.__print__('Process ' + _this._proc.pid + ' started\n');
    return _this;
  }

  _createClass(Shell, [{
    key: '__print__',
    value: function __print__(msg) {
      this._cfg.debugMsg && console.log('' + msg);
    }
  }, {
    key: 'addCommand',
            ...
            if (_type === 'Object') {
              var _ret = function () {
                ...
                if (_keys.length === 2 && _keys[0] === 'name' && _keys[1] === 'value') {
                  ...
                  return {
                    v: reject('All objecct params need to be {name: \'\', value: \'\'} or {name: value} structure')
                  };
                }
              ...
              return reject('All Params need to be objects or strings');
            }
          }
        } catch (err) {
    ...
    value: function invoke() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _self.__print__('Command invoke started');
        _self._cfg.debugMsg && console.log(' ' + _cmdsStr);

        function resolve_listener(data) {
          _self.__print__('Command invoke finished\n');
          reset();
          return resolve(data);
        }
        function reject_listener(error) {
          _self.__print__('Command invoke failed\n');
          reset();
          return reject(error);
        }
        function reset() {
    ...
    value: function dispose() {
      ...
      return new Promise(function (resolve, reject) {
        _self._proc.on('close', function (code) {
          ...
          _self.emit('end', code);
          if (code == 1) {
            _self.__print__(_exitMsg);
            return reject('script exit ' + code);
          } else {
            _self.__print__(_exitMsg);
